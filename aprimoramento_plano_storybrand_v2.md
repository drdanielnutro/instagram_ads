...
- **Disponibilidade dos Arquivos:** A criação desses prompts é parte integrante da implementação do fallback. O `PromptLoader` descrito na Seção 16.4 opera em modo fail-fast: no momento da inicialização ele carrega e valida **todos** os arquivos obrigatórios e dispara `FileNotFoundError` se algum estiver faltando ou com nome incorreto. Durante desenvolvimento inicial, mantenha `config.enable_storybrand_fallback=False` (ou o toggle `ENABLE_STORYBRAND_FALLBACK=false`) até que os prompts estejam provisionados para evitar a inicialização antecipada do fallback.
